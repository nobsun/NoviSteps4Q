...
solve :: Solver
solve = \ case
    as -> bool "No" "Yes" $ any upper $ combinations 2 $ phi <$> as
        where
            phi :: [Int] -> (Int,IS.IntSet)
            phi (p:_:fs) = (p,IS.fromList fs)
            phi _        = invalid 

upper :: [(Int,IS.IntSet)] -> Bool
upper = \ case
    [(p,fs), (q,gs)] -> case p `compare` q of
        LT -> gs `IS.isSubsetOf` fs
        GT -> fs `IS.isSubsetOf` gs
        EQ -> fs `IS.isProperSubsetOf` gs || gs `IS.isProperSubsetOf` fs
    _ -> invalid
