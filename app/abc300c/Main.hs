...
solve :: Solver
solve = \ case
    ((h,w),ss) -> drop 1 $ M.elems m where
        m = foldl' phi m0 rng
        rng = range ((1,1),(h,w))
        m0  = M.fromList $ zip [0 .. min h w] (repeat 0)
        phi ms (x,y) = M.insertWith (+) (count (x,y)) 1 ms
        count (x,y) = bool 0 (iter 1) ('#' == ba ! (x,y))  where
            iter d
                | (x - d) * (y - d) == 0 = pred d
                | subtract (x + d) (succ h) * subtract (y + d) (succ w) == 0 = pred d
                | otherwise 
                    = bool (pred d) (iter (succ d)) 
                    $ all ('#' ==) 
                    $ map (ba !) [ (x+i,y+j) | i <- [-d,d], j <- [-d,d]]
        ba = listArray ((1,1),(h,w)) $ concat ss
    
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    hw:ss -> case f (toTuple $ map read $ words hw, ss) of
        r -> [r]
