...
type Solver = (Int,I,[(Int,Int)]) -> [O]

solve :: Solver
solve = \ case
    (n,s,qs) -> case zipWith phi s (tail s) of
        as -> case listArray (0,pred n) $ scanl (+) 0 as of
            aa -> map (psi aa) qs
            where
                psi a (i,j) = a ! pred j - a ! pred i
        where
            phi p q = bool 0 1 $ p == q

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_]:[s]:lrs -> case map (toTuple . map read) lrs of
        lrs' -> case f (read n,s,lrs') of
            r -> (:[]) <$> r
