...
solve :: Solver
solve = \ case
    (as,qqs) -> snd $ mapAccumL phi (sum as, ms0) qqs where
        ms0 :: M.Map Int Int
        ms0 = M.fromList $ runLength $ sort as
        phi :: (Int, M.Map Int Int) -> [Int] -> ((Int, M.Map Int Int), Int)
        phi (s,ms) = \ case
            [b,c] -> maybe ((s,ms),s) psi (ms M.!? b)
                where
                    psi :: Int -> ((Int, M.Map Int Int), Int)
                    psi n = ((s', M.delete b (M.insertWith (+) c n ms)), s')
                        where
                            s' = s + subtract b c * n
            _     -> invalid
