...
type Dom   = (Int,Int,String,[(Int,Int)])
type Codom = O
...
solve = \ case
    (h,k,s,xys) -> case s of
            'R':r -> iter ps (1,0) (pred h) r
            'L':r -> iter ps (-1,0) (pred h) r
            'U':r -> iter ps (0,1) (pred h) r
            'D':r -> iter ps (0,-1) (pred h) r
            _     -> invalid
        where
            ps = S.fromList xys
            iter :: (S.Set (Int,Int)) -> (Int,Int) -> Int -> [Char] -> O
            iter ps_ (x,y) h_ = \ case
                rs | h_ < 0    -> "No"
                   | otherwise -> case rs of
                    []    -> "Yes"
                    'R':r -> iter ps' (succ x,y) h' r
                    'L':r -> iter ps' (pred x,y) h' r
                    'U':r -> iter ps' (x,succ y) h' r
                    'D':r -> iter ps' (x,pred y) h' r
                    _     -> invalid
                where
                    (ps',h')
                        | S.member (x,y) ps_ && h_ < k
                                    = (S.delete (x,y) ps_, pred k)
                        | otherwise = (ps_, pred h_)

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,_,h,k]:[s]:xys -> case f (read h, read k, s, map (toTuple . map read) xys) of
        r -> [[r]]
