...
solve :: Solver
solve = \ case
    as -> case tail $ scanl phi ((0,0) :: (Int,Int), IM.empty :: IM.IntMap Int) $ (,1) <$> as of
        es -> fst . fst <$> es
        where
            phi :: ((Int,Int), IM.IntMap Int) -> (Int,Int) -> ((Int,Int), IM.IntMap Int)
            phi ((i,v),m) (j,o) = case IM.insertWith (+) j o m of
                m' -> case fromJust $ IM.lookup j m' of
                    u -> case v `compare` u of
                        LT -> ((j,u),m')
                        GT -> ((i,v),m')
                        EQ -> (bool (i,v) (j,u) (i > j), m')

cmp :: (Int,Int) -> (Int,Int) -> Ordering
cmp (i,x) (j,y) = case compare x y of
    EQ -> compare j i
    r  -> r
