...
solve :: Solver
solve = \ case
    (n,as) -> case listArray ((0,0),(pred n,pred n)) $ concat as of
        aa     -> maximum (maxFollowing n aa <$> range ((0,0),(pred n,pred n)))

maxFollowing :: Int -> Array (Int,Int) Char -> (Int,Int) -> String
maxFollowing n a ix = maximum (map (map (a !)) (allFollowings n ix))

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    n:as -> case f (read n, as) of
        rr -> [rr]
...
neighbors :: [Neighbor]
neighbors = [minBound .. maxBound]

neighborOf :: Int -> Neighbor -> (Int,Int) -> (Int,Int)
neighborOf n@(_m+1) = \ case
    N -> first  ((`mod` n) . pred)
    E -> second ((`mod` n) . succ)
    S -> first  ((`mod` n) . succ)
    W -> second ((`mod` n) . pred)
    NE -> neighborOf n N . neighborOf n E
    SE -> neighborOf n S . neighborOf n E
    SW -> neighborOf n S . neighborOf n W
    NW -> neighborOf n N . neighborOf n W
neighborOf _ = error "neighborOf: not positive integer"

followings :: Int -> Neighbor -> (Int,Int) -> [(Int,Int)]
followings n d = take n . iterate (neighborOf n d)

allFollowings :: Int -> (Int,Int) -> [[(Int,Int)]]
allFollowings n (i,j) = map (flip (followings n) (i,j)) neighbors
