...
type Solver = (I,[I],[[I]]) -> Int

solve :: Solver
solve = \ case
    (n,ks,xys) -> case listArray (1,n) xys of
        as         -> case IS.toList (IS.fromList [1 .. n] IS.\\ IS.fromList ks) of
            bs         -> maximum [ minimum $ phi i <$> ks | i <- bs ]
                where
                    phi i j  = sum $ square <$> zipWith (-) (as ! i) (as ! j)
                    square x = x * x

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_]:ks:xys -> case f (n,ks,xys) of
        r -> [[sqrt (fromIntegral r)]]
