...
type Solver = (IS.IntSet, IS.IntSet,[I]) -> [O]

solve :: Solver
solve = \ case
    (a,b,c) -> IS.toList $ case c of
        "intersection":_ -> aintb
        "union_set":_    -> aunib
        "symmetric_diff":_ -> aunib IS.\\ aintb
        "subtract":x:_     -> read x `IS.delete` a
        "increment":_      -> IS.map (flip mod 50 . succ) a
        "decrement":_      -> IS.map (flip mod 50 . pred) a
        _ -> invalid
        where
            aintb = a `IS.intersection` b
            aunib = a `IS.union` b


wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    _:as:_:bs:c:_ -> case f (IS.fromList $ read <$> as
                            ,IS.fromList $ read <$> bs
                            ,c) of
        r -> [r]
