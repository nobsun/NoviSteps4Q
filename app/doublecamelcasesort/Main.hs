...
solve :: Solver
solve = \ case
    s -> case map toTuple $ splitEvery 2 $ findIndices isUpper s of
        ixs -> concat $ sortBy (comparing (map toLower)) $ unfoldr phi (ixs, s) where
            phi ([],_) = Nothing
            phi (is,cs) = case is of
                (i,j):_ -> case splitAt (succ (j-i)) cs of
                    (as,bs) -> Just (as, (tail is, bs))
