...
solve :: Solver
solve = \ case
    (n,ccs) -> case G.buildG (1,n) (dup =<< ccs) of
        gr -> map phi (indices $ gr)
            where
                phi v = case gr ! v of
                    vs -> S.size $ S.delete v $ flip S.difference vss $ S.unions $ map psi vs where
                        psi u = S.fromList $ gr ! u
                        vss   = S.fromList vs
    where
        dup (x,y) = [(x,y),(y,x)]
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_m]:ccs -> case f (n,toTuple <$> ccs) of
        rr -> singleton <$> rr
