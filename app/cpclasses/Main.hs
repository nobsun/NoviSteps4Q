...
solve :: Solver
solve = \ case
    (as,bs) -> case IS.fromList as of
        aa -> map phi bs
            where
                phi = \ case
                    b | IS.member b aa -> 0
                      | otherwise -> case (IS.lookupLT b aa, IS.lookupGT b aa) of
                        (Nothing,Just ax) -> ax - b
                        (Just an,Nothing) -> b - an
                        (Just an,Just ax) -> min (ax - b) (b - an)
                        _ -> invalid

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    _:as:_:bss -> case f (as,head <$> bss) of
        rr -> (:[]) <$> rr
...
head :: [a] -> a
head = list (error "head: empty list") const 

tail :: [a] -> [a]
tail = list (error "tail: empty list") (const id)
