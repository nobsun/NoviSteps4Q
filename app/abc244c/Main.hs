...
import qualified System.Posix.Internals as B
...
solve :: Solver
solve = \ case
    (n,as) -> case S.fromList [2 .. 2*n+1] of
        ss     -> 1 : iter ss as where
            iter s = \ case
                []   -> invalid
                0:_  -> []
...
iterLoop s = do
    { i <- readInt <$> B.getLine
    ; if i == 0 then return ()
      else case S.deleteFindMin (S.delete i s) of
            (j,t) -> do 
                { B.putStrLn (showInt j)
                ; hFlush stdout
                ; iterLoop t 
                }
    }

interactWithFlush :: (B.ByteString -> B.ByteString) -> IO ()
interactWithFlush f = loopIter . B.lines . f =<< B.getContents
    where
        loopIter lls = do
            { case lls of
                []   -> return ()
