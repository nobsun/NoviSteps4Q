...
solve :: Solver
solve = \ case
    s -> iter (IS.singleton 0) s where
        iter :: IS.IntSet -> String -> Int
        iter ss = \ case
            [] -> IS.findMax ss
            cs -> case spanCount ('1' ==) cs of
                (m, '/':cs1) -> case spanCount ('2' ==) cs1 of
                    (n,cs2)      -> case compare m n of
                        GT -> iter (IS.insert (2*n+1) ss) cs2
                        _  -> iter (IS.insert (2*m+1) ss) cs2
                (_,cs1) -> iter ss (dropWhile ('2' ==) cs1)
