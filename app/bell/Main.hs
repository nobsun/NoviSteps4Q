...
solve :: Solver
solve = \ case
    (k,as,bs) -> case IS.fromList as of
        aa -> map phi bs where
            phi b = case (IS.lookupLE b aa, IS.lookupGE b aa) of
                (Just l, Just r) -> max 0 $ k - min (b - l) (r - b)
                (Just l, _)      -> max 0 $ k - (b - l)
                (_, Just r)      -> max 0 $ k - (r - b)
                _                -> invalid

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,_,k]:as:bs:_ -> case f (k,as,bs) of
        r -> (:[]) <$> r
