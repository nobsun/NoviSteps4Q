...
solve :: Solver
solve = \ case
    (h,w,d,ss) -> case listArray ((1,1),(h,w)) (tracing $ concatMap phi ss) of
        sa -> case listArray ((1,1),(h,w)) (tracing $ psi <$> assocs sa) of
            sb -> maximum $ map xsi $ combinations 2 $ tracing $ mapMaybe xi $ assocs sb
                where
                    xi (ix,ns) = bool (Just ix) Nothing (S.null ns)
                    xsi = \ case 
                        [ix,jx] -> S.size $ (sb ! ix) `S.union` (sb ! jx)
                        _       -> impossible
            where
                psi = \ case
                    ((i,j),b) 
                        | not b     -> S.empty
                        | otherwise -> S.fromList [ (i',j')
                                                  | di <- [-d .. d], dj <- [abs di - d .. d - abs di]
                                                  , let {i' = i+di; j'=j+dj}
                                                  , 0 < i', 0 < j' , i' <= h, j' <= w, sa ! (i',j')]
        where
            phi = map ('.' ==)

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [h,w,d]:ss -> case f (read h, read w, read d, concat ss) of
        r -> [[r]]
